...
import re
import openai
import requests  # Used for direct API calls to Assistants API v1
import logging
import json
...
# Assistant IDs for gpt-4o-mini and gpt-4o
ASSISTANT_IDS = {
...
"""
    ...
    print("Message sent to the assistant for model selection:")  # Print 1
    ...
    try:
        ...
        response = openai.ChatCompletion.create(
            ...
            max_tokens=3500,  # Adjusted max tokens
            temperature=0  # For deterministic output
        )

        assistant_response = response['choices'][0]['message']['content'].strip()
        print("Assistant response for model selection:")  # Print 2
        ...
        print(f"Using model: {selected_model}")  # Print 3
...
def generate_response(user_text, selected_model):
    try:
        ...
        print("Message sent to the assistant for response generation:")  # Print 1
        print(f"User: {user_text}")

        if selected_model in ['o1-mini', 'o1-preview']:
            # Implement o1 models without setting `temperature`
            messages = [
                {
                    "role": "user",
                    "content": [
                        {
                            "type": "text",
                            "text": user_text  # Only the user's message
                        }
                    ]
                }
            ]
            response = openai.ChatCompletion.create(
                ...
                messages=messages,
                max_completion_tokens=15000  # As per your requirement
            ...
            # Implement assistant-based API calls for gpt-4o and gpt-4o-mini using Assistants API v1
            ...
            run_data = {
                ...
                # Do not set 'instructions' if assistant already has it
            }
        ...
        print("Assistant response:")  # Print 4
